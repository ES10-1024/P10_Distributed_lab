...
from SSSS02_plot_communication import SSSS
...
class ADMM_optimiser_WDN:
    ...
    def optimise(self, hour : int , water_height: float):
        ...
        while k < self.max_iterations:
            ...
            self.z = self.smpc_summer.sum(self.z_i,True ) / self.N_s  #SMPC calculation of z
           ...
           ### BEGIN find rho
            if(k<=self.N_vary_rho):
                ...
                self.x_bar = 1/self.N_s*self.smpc_summer.sum(self.x_i, False)
                
                self.r_i = np.linalg.norm(self.x_i - self.x_bar, 2)**2
                
                self.r_norm_squared =  self.smpc_summer.sum(self.r_i, False)
            ...
            if k >= self.stop_criterion_start - 1 and k % self.n_iteration_stop_criteria == self.n_iteration_stop_criteria-1:
                self.x_bar_old = 1/self.N_s*self.smpc_summer.sum(self.x_i, False)
