...
ones_NcNq_NcNq = np.ones((c_general['N_c']*c_general['N_q'], c_general['N_c']*c_general['N_q']))


def define_Jp(pump_constants, A_3, J_e, h_V, d):

    #Elevation and water height
    elevation = 1/pump_constants['eta'] * (c_tower['rho_w']*c_tower['g_0']/c_general['condition_scaling'] * pump_constants['h']*( A_3 @ Uc))
    water_height = 1/pump_constants['eta'] * (c_tower['rho_w']*c_tower['g_0']/c_general['condition_scaling']*(h_V*(A_3 @ Uc)))

    #Pipe resistances
    pipe_resistance = 1/pump_constants['eta']*(pump_constants['r_f']/c_general['condition_scaling']*(A_3 @ (Uc * ca.fabs(Uc)*ca.fabs(Uc))))
    combined_pipe_resistance = 1/pump_constants['eta']*(c_general['r_fsigma']/c_general['condition_scaling'] * ca.fabs((A_1 @ Uc)-d)*((A_1 @ Uc)-d)*(A_3 @ Uc))                                                        
    
    #Total power consumption
    P = pipe_resistance + combined_pipe_resistance + water_height + elevation
...
#Must be defined before cost function can be found
    ...
    h_V = V_0/c_tower['A_t'] + (c_general['t_s']*(A_2 @ (q_sigma * c_general['hours_to_seconds'])))/c_tower['A_t']
...
#Constraints
    ...
    b_tower_min_vol = - c_tower['V_min'] * ones_Nc + V_0*ones_Nc - A_2*c_general['t_s'] @ d /3600
    
    #Maximum volume in tower
    A_tower_max_vol = A_2 @ A_1 * c_general['t_s']/3600
    b_tower_max_vol = c_tower['V_max'] * ones_Nc - V_0*ones_Nc + A_2*c_general['t_s'] @ d /3600
