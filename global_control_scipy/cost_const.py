...
def define_Jp(pump_constants, A_3, J_e, h_V, d):
    ...
    elevation = lambda Uc:   ((c_tower['rho_w']*c_tower['g_0'])/c_general['condition_scaling'])*(pump_constants['h']* (A_3 @ Uc))
    water_height = lambda Uc: ((c_tower['rho_w']*c_tower['g_0'])/c_general['condition_scaling'])*(h_V(Uc)*(A_3 @ Uc))

    #Pipe resistances
    pipe_resistance = lambda Uc: pump_constants['r_f']/c_general['condition_scaling']*(A_3 @ (Uc * np.abs(Uc)*np.abs(Uc)))
    combined_pipe_resistance = lambda Uc: (A_3 @ Uc) * (c_general['r_fsigma']/c_general['condition_scaling'] * (np.abs((A_1 @ Uc)-d))*((A_1 @ Uc)-d))                                                    
    
    #Total power consumption
    P = lambda Uc: 1/pump_constants['eta'] * (pipe_resistance(Uc) + combined_pipe_resistance(Uc) + water_height(Uc) + elevation(Uc))
    #Final cost function for J_p
    J_p = lambda Uc: J_e.T @ P(Uc)
...
#Must be defined before cost function can be found
    #Flow into the tower
    q_sigma = lambda Uc: (A_1 @ (Uc* c_general['hours_to_seconds']*c_general['t_s']))  - (d* c_general['hours_to_seconds']*c_general['t_s']) 

    #Height of water in the tower
    h_V = lambda Uc: 1/c_tower['A_t']* (A_2 @ q_sigma(Uc) + V_0)

#Cost function

    #Cost function for difference in tower before and after
    J_V = lambda Uc: c_tower['kappa']*((c_general['t_s']*ones_Nc.T) @ (A_1 @ (Uc * c_general['hours_to_seconds'])-(d* c_general['hours_to_seconds'])))**2

    J_p_1 =  define_Jp(c_pump1, A_31, J_e, h_V, d) 
    J_p_2 =  define_Jp(c_pump2, A_32, J_e, h_V, d)

    J_k = lambda Uc: J_p_1(Uc) +J_p_2(Uc) + J_V(Uc)
